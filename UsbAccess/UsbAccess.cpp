...
namespace Plugin {
    ...
    , _controller(nullptr)
    , _usbMassStorageObject(nullptr)
    , _usbMassStorageNotification(*this)
    ...
    void UsbAccess::Deinitialize(PluginHost::IShell * /* service */)
    {
        ...
        if(_usbMassStorageObject)
        {
            _usbMassStorageObject->Release();
        }
        if (_controller)
        {
            _controller->Release();
    ...
    void UsbAccess::registerEventHandlers()
    {
        ASSERT (nullptr != _usbMassStorageObject);
        ASSERT(false == _registeredEventHandlers);
    
        if(!_registeredEventHandlers && _usbMassStorageObject) {
            _registeredEventHandlers = true;
            _usbMassStorageObject->Register(&_usbMassStorageNotification);
        }
    }
    ...
    void UsbAccess::InitializeUSBMassStorage()
    {
        ...
        _engine->Announcements(_communicatorClient->Announcement());
            ...
            _controller = _communicatorClient->Open<PluginHost::IShell>(_T("org.rdk.UsbMassStorage"), ~0, 3000);
            if (_controller)
            {
                 _usbMassStorageObject = _controller->QueryInterface<Exchange::IUSBMassStorage>();

                 if(_usbMassStorageObject)
                 {
                     _usbMassStorageObject->AddRef();
                 }
            }
            else
            {
                LOGERR("Failed to create USBMassStorage Controller\n");
            }
        }
    ...
    void UsbAccess::Dispatch(const bool ismounted, const string mount_path)
    {
         _adminLock.Lock();
         onUSBMountChanged(ismounted, mount_path);
         _adminLock.Unlock();
    }
    void UsbAccess::onDeviceMounted(const Exchange::IUSBMassStorage::USBStorageDeviceInfo &deviceInfo, Exchange::IUSBMassStorage::IUSBStorageMountInfoIterator* const mountPoints)
    {
        Exchange::IUSBMassStorage::USBStorageMountInfo usbMountInfo;
        _adminLock.Lock();
        // making mount paths are empty.
        m_paths.clear();

        if (mountPoints)
        {
            while (mountPoints->Next(usbMountInfo) == true)
            {
                Core::IWorkerPool::Instance().Submit(Job::Create(this, true, usbMountInfo.mountPath));
            }
        }
       _adminLock.Unlock();
    }
    void UsbAccess::onDeviceUnmounted(const Exchange::IUSBMassStorage::USBStorageDeviceInfo &deviceInfo, Exchange::IUSBMassStorage::IUSBStorageMountInfoIterator* const mountPoints)
    {
        Exchange::IUSBMassStorage::USBStorageMountInfo usbMountInfo;
        _adminLock.Lock();
        // making mount paths are empty.
        ...
        if (mountPoints)
        {
            ...
            while (mountPoints->Next(usbMountInfo) == true)
            {
                Core::IWorkerPool::Instance().Submit(Job::Create(this, false, usbMountInfo.mountPath));
            }
        }
        _adminLock.Unlock();
    }

    void UsbAccess::onUSBMountChanged(bool mounted, string mount_path)
    {
        JsonObject params;
        params["mounted"] = mounted;
        params["device"] = mount_path;
    ...
    bool UsbAccess::getMounted(std::list <std::string>& paths)
    {
        ...
        {
            ASSERT (nullptr != _usbMassStorageObject);
            if (nullptr != _usbMassStorageObject)
            {
                status = _usbMassStorageObject->GetDeviceList(deviceInfo);
                if ((status == Core::ERROR_NONE) && (deviceInfo != nullptr))
                {
                    Exchange::IUSBMassStorage::USBStorageDeviceInfo usbDeviceInfo;

                    LOGWARN("usbDeviceInfo.deviceName = %s", usbDeviceInfo.deviceName.c_str());

                    while (deviceInfo->Next(usbDeviceInfo) == true)
                    {
                        Exchange::IUSBMassStorage::IUSBStorageMountInfoIterator* itr_mountPoints = nullptr;
                        status = _usbMassStorageObject->GetMountPoints(usbDeviceInfo.deviceName, itr_mountPoints);

                        if ((status == Core::ERROR_NONE) && (itr_mountPoints != nullptr))
                        {
                            Exchange::IUSBMassStorage::USBStorageMountInfo usbMountInfo;
                            while (itr_mountPoints->Next(usbMountInfo) == true)
                            {
                                m_paths.emplace_back(usbMountInfo.mountPath);
                                LOGWARN("usbMountInfo.mountPath = %s", usbMountInfo.mountPath.c_str());
                            }
                            itr_mountPoints->Release();
                        }
                        else
                        {
                            LOGWARN("MountPoints are empty");
                        }
                    }
                    deviceInfo->Release();
                }
                ...
                {
                    LOGWARN("GetDeviceList is empty");
                }
            }
            else
            {
                LOGWARN("_usbMassStorageObject is null");
            }
        }
        if (!m_paths.empty())
        {
            paths.assign(m_paths.begin(),m_paths.end());
            result = true;
        }
        return result;
    }
